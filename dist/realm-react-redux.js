(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('realm'), require('react')) :
	typeof define === 'function' && define.amd ? define(['exports', 'realm', 'react'], factory) :
	(factory((global.ReactRedux = global.ReactRedux || {}),global.Realm,global.React));
}(this, (function (exports,Realm,react) { 'use strict';

Realm = Realm && 'default' in Realm ? Realm['default'] : Realm;
...
function createRealmStore(writer, options, enhancer) {
    ...
    var realm = options.realm,
        ...
        realmOptions = objectWithoutProperties(options, ['realm', 'allowUnsafeWrites', 'watchUnsafeWrites']);
...
function combineWriters(writers) {
    var finalWriters = [];
    writers.forEach(function (writer) {
        if (typeof writer === 'function') {
...
function applyMiddleware() {
    ...
    return function (createRealmStore) {
        return function (writer, realmOptions, enhancer) {
            var store = createRealmStore(writer, realmOptions, enhancer);
            var _dispatch = store.dispatch;
            var chain = [];

            var middlewareAPI = {
                getRealm: function getRealm() {
                    return store.realm;
                },
...
var subscriptionShape = index$2.shape({
    trySubscribe: index$2.func.isRequired,
    tryUnsubscribe: index$2.func.isRequired,
    notifyNestedSubs: index$2.func.isRequired,
    isSubscribed: index$2.func.isRequired
});

var storeShape = index$2.shape({
    subscribe: index$2.func.isRequired,
    dispatch: index$2.func.isRequired,
    getRealm: index$2.func.isRequired
});
...
function makeSelectorStateful(sourceSelector, store) {
    ...
    var selector = {
        run: function runComponentSelector(props) {
            try {
                var nextProps = sourceSelector(store.getRealm(), props);
        ...
        cleanup: sourceSelector.cleanup
    };
    return selector;
}
...
selectorFactory) {
    ...
    var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
        ...
        storeKey = _ref$storeKey === undefined ? 'realmStore' : _ref$storeKey,
    ...
    return function wrapWithConnect(WrappedComponent) {
        invariant_1(typeof WrappedComponent === 'function', 'You must pass a component to the function returned by ' + ('connect. Instead received ' + JSON.stringify(WrappedComponent)));
...
function verifyArray(value, displayName, methodName) {
        ...
        proxy.mapToProps = function detectFactoryAndVerify(realmOrDispatch, ownProps) {
...

            {
                if (methodName === 'mapPropsToQueries') {
                    verifyArray(props, displayName, methodName);
                } else {
...
var defaultMapPropsToQueriesFactories = [whenMapPropsToQueriesIsFunction, whenMapPropsToQueriesIsMissing];
...
var defaultMapQueriesToPropsFactories = [whenMapQueriesToPropsIsFunction, whenMapQueriesToPropsIsMissing];
...
function impureFinalPropsSelectorFactory(mapPropsToQueries, mapQueriesToProps, mapDispatchToProps, mergeProps, dispatch) {
    return function impureFinalPropsSelector(realm, ownProps) {
        return mergeProps(mapQueriesToProps(mapPropsToQueries(realm, ownProps), ownProps), mapDispatchToProps(dispatch, ownProps), ownProps);
    };
}

function pureFinalPropsSelectorFactory(mapPropsToQueries, mapQueriesToProps, mapDispatchToProps, mergeProps, dispatch, _ref) {
    ...
    function handleNewState() {
        var nextQueryProps = mapQueriesToProps(queries, ownProps);
...
function finalPropsSelectorFactory(dispatch, _ref3) {
    var initMapPropsToQueries = _ref3.initMapPropsToQueries,
        ...
        options = objectWithoutProperties(_ref3, ['initMapPropsToQueries', 'initMapQueriesToProps', 'initMapDispatchToProps', 'initMergeProps']);
...
function createConnect() {
    ...
    return function connect(mapPropsToQueries, mapQueriesToProps, mapDispatchToProps, mergeProps) {
        var _ref2 = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {},
            ...
            extraOptions = objectWithoutProperties(_ref2, ['pure', 'areOwnPropsEqual', 'areQueryPropsEqual', 'areMergedPropsEqual']);
...
function createRealmProvider() {
    ...
    Provider.propTypes = {
        ...
        children: index$2.element.isRequired
    };
...
var provider = createRealmProvider();
...
exports.createRealmStore = createRealmStore;
exports.combineWriters = combineWriters;
exports.bindActionCreators = bindActionCreators;
exports.applyMiddleware = applyMiddleware;
exports.compose = compose;
exports.RealmProvider = provider;
exports.realmConnect = connect;
