...
exports.default = combineWriters;
...
function assertWriterBehavior(writers) {
    writers.forEach(function (writer) {
        ...
        writer(realm, { type: _store.ActionTypes.INIT });

        realm = new FakeRealm('writer "' + writer.name + '" operated on realm when probed with a random type.' + ('Don\'t try to handle ' + _store.ActionTypes.INIT + ' or other actions in "realm-redux/*" ') + 'namespace. They are considered private.');
        var type = _store.ActionTypes.PROBE_UNKNOWN_ACTION + '_' + Math.random().toString(36);
